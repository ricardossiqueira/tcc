...
export default function ContainerList() {
  ...
  } = useGetUserContainersQuery<IContainerList[]>();
  ...
  const startContainerMutation = useMutation({
    ...
    onMutate: async (id) => {
      ...
      queryClient.setQueryData(
        getUserContainersQueryKey,
        (oldData: IContainer[]) =>
          oldData.map((container) =>
            container.id === id ? { ...container, isLoading: true } : container,
          ),
      );
        ...
        <TableBody>
          {sortedContainers.map((container) => (
            <TableRow key={container.id}>
              ...
              <TableCell>
                {toDate(container.created).toLocaleDateString()}
              </TableCell>
